...
TOL = 1e-6
POINTS = [(np.deg2rad(a), ct) for a in (-5.0, 5.0, 14.0, 25.0)
...
def main():
    ...
    failures, worst, worst_n = 0, 0.0, None
    for coef, term, ev, esperado, orden in tests:
        bad = 0
        for al, ct in POINTS:
            base = ev(al, ct, 0.0)
            if orden == 1:
                # CENTRED: the forward difference would drag the quadratic term
            ...
            if rel > worst:
                worst, worst_n = rel, "%s by %s" % (coef, term)
            if rel > TOL:
                bad += 1
                if bad == 1:
                    print("  DIFFERS  %s by %-6s  alpha %+5.1f C_T %.2f: "
    ...
    print("\n%d terminos probados en %d puntos (alpha, C_T), %d difieren "
          "(worst %.2e in %s)" % (len(tests), len(POINTS), failures, worst, worst_n))
