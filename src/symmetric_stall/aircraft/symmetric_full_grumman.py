...
Moving the CG. Riley refers every coefficient to a CG on the fuselage
centreline at 0.25 c_bar (report p. 5 and Table I). A run with the CG
elsewhere keeps Riley's aircraft exactly as published -- same mass, same
Iyy, same tables -- and moves only the reference point. That is a controlled
static-margin study, not a re-loaded aeroplane: Iyy is deliberately NOT
adjusted, because adjusting it would require saying how the mass was
redistributed and would confound the effect being measured.

The transfer of the aerodynamic moment to the displaced CG is exact for any
force system, and is applied to the TOTAL normal force -- including the parts
generated by q and alpha_dot -- not just the static part. It is verified three
ways in scripts/verify/verificar_traslado_cg.py and matches the CUDA kernel to
1e-5 with the CG shifted (checked at +5% and -7% of chord).

One second-order effect is NOT modelled, and it is aerodynamic rather than
inertial, so keeping Riley's parameters does not remove it. C_Lq, C_mq, C_Lalpha_dot
and C_malpha_dot are measured for rotation about the reference CG. With the CG
displaced by dx (aft-positive) the tail arm becomes l_t - dx, so those
derivatives should shrink by (1 - dx/l_t), with l_t = 11.62 ft = 3.542 m from
Table I. The moment transfer already supplies ONE factor of (1 - dx/l_t): the
code's effective C_mq is C_mq(ref)(1 - dx/l_t) where the correct value is
C_mq(ref)(1 - dx/l_t)^2. The residual is dx/l_t, i.e.

    CG shift      residual error on C_Lq and C_mq
      3% chord              1.0 %
      5% chord              1.8 %
      7% chord              2.5 %

The sign matters for how the result is read: for an AFT CG the code carries
slightly MORE pitch damping than the aircraft has, so it marginally understates
how hard an aft CG is to recover. Correcting it would mean layering a
tail-only model (constant a_t, S_t/S, single tail arm) on top of Riley's
whole-aircraft wind-tunnel derivatives, trading a known 2% bias for an
assumption that cannot be validated against the report. It is left declared
rather than patched.
